...
def build_base_stochastic_dispatch_scenario(
    ...
    network.scenario_description = (
        f"Five-sample solar stochastic dispatch with {nonanticipative_hours:g}-hour "
        "generation non-anticipativity for Bergen, LM6000, and Titan130 thermal generators."
    )
...
def add_regulation_raise_reserve_constraints(
    ...
    model.add_constraints(
        ...
        name="Regulation-raise-min-provision",
    )
    model.add_constraints(
    ...
    if network.regulation_raise_reserve_dynamic_risk:
        for contingency in network.regulation_raise_reserve_contingencies:
            ...
            model.add_constraints(
                ...
                name=f"Regulation-raise-dynamic-risk-{contingency}",
            )
...
def add_stochastic_regulation_raise_reserve_constraints(
    ...
    model.add_constraints(
        provision + shortage >= network.regulation_raise_reserve_min_provision_mw,
        name=f"Regulation-raise-min-provision-{sample}",
    )
    model.add_constraints(
        provision <= network.regulation_raise_reserve_max_provision_mw,
...
def export_reserves_by_interval(
    ...
    reserve_specs = (
        ("Inertia", "Inertia-shortage-{sample}", None),
        (
            "Regulation Lower",
...
def export_line_flow_by_interval(
    network: pypsa.Network,
    output_path: Path | str,
    snapshots: pd.DatetimeIndex | None = None,
) -> pd.DataFrame:
    ...
    p1 = network.lines_t.p1 if hasattr(network.lines_t, "p1") else None

    for component_name, line in network.lines.iterrows():
        ...
        bus0 = (
            str(network.buses.loc[bus0_component].get("base_name", bus0_component))
            if bus0_component in network.buses.index
            else str(bus0_component)
        )
        bus1 = (
            str(network.buses.loc[bus1_component].get("base_name", bus1_component))
            if bus1_component in network.buses.index
            else str(bus1_component)
        )
        limit_mw = float(line.get("s_nom", 0.0) or 0.0)
        series0 = p0[component_name].reindex(snapshots)
        series1 = (
            p1[component_name].reindex(snapshots)
            if p1 is not None and component_name in p1.columns
            else None
        )
        for snapshot, value0 in series0.items():
            p0_mw = float(value0) if pd.notna(value0) else 0.0
            p1_mw = (
                float(series1.loc[snapshot])
                if series1 is not None and pd.notna(series1.loc[snapshot])
                else ""
            )
            rows.append(
                {
                    ...
                    "p1_mw": p1_mw,
                    "limit_mw": limit_mw,
                    "loading_pct": abs(p0_mw) / limit_mw * 100.0 if limit_mw else "",
                }
            )
...
def export_base_stochastic_solution(
    ...
    outputs = {
        ...
        "line_flow": output_dir / "line_flow_by_interval.csv",
    }
    ...
    export_line_flow_by_interval(network, outputs["line_flow"])
...
def assert_base_stochastic_dispatch_scenario(
    network: pypsa.Network,
    expected_nonanticipative_hours: float = GENERATION_NONANTICIPATIVE_HOURS,
) -> None:
    ...
    if network.generation_nonanticipative_hours != expected_nonanticipative_hours:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=GENERATION_NONANTICIPATIVE_HOURS,
        help=(
            "Hours from the start of the optimization for which thermal generator "
            "dispatch and commitment are tied across stochastic samples."
        ),
    )
    parser.add_argument(
        "--solver-name",
...
def main() -> int:
    ...
    if args.generation_nonanticipative_hours < 0:
        raise SystemExit("error: --generation-nonanticipative-hours must be zero or greater")
    ...
    print(f"  generation non-anticipativity: {args.generation_nonanticipative_hours:g} hours")
    if solver_time_limit is not None:
    ...
    solved_network, result = solve_base_stochastic_dispatch_scenario(
        ...
        data_dir=Path(args.data_dir),
        nsj_sf_year=args.nsj_sf_year,
        nonanticipative_hours=args.generation_nonanticipative_hours,
        gurobi_license_file=gurobi_license_file,
        solver_time_limit=solver_time_limit,
        solver_mip_gap=solver_mip_gap,
        solver_threads=solver_threads,
    )
    assert_base_stochastic_dispatch_scenario(
        solved_network,
        expected_nonanticipative_hours=args.generation_nonanticipative_hours,
    )
    status, condition = result
